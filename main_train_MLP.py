...
HIDDEN_OBS = False       # True: chaser-only observation (target/obstacle hidden)
HIDDEN_RL2 = False       # True: also feed previous action+reward (RL^2) to the LSTM
...
if __name__ == "__main__":
    ...
    MLPtype       = 'PPO'       # 'PPO' or 'RPPO' (RecurrentPPO/LSTM)
    rnn           = MLPtype == 'RPPO'
    ...
    if rnn:   # the ONLY LSTM config that escaped 1/5 (reached 3229) -- resume & finish it.
        learning_rate = 3e-4    #   (every calibrated lr/n_steps/lstm variant stalled at 1/5)
        n_epochs      = 10
        n_steps       = 256 * num_env
